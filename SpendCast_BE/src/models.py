...
class CustomerUpdate(BaseModel):
    ...
    name: Optional[str] = Field(None, min_length=1, max_length=255, description="Customer full name")
    email: Optional[str] = Field(None, description="Email address")
    phone: Optional[str] = Field(None, description="Phone number")
    birth_date: Optional[date] = Field(None, description="Birth date")
    citizenship: Optional[str] = Field(None, description="Citizenship")
...
class ReceiptDetails(ReceiptBase):
    ...
    items: List[ReceiptItemDetails] = Field(default_factory=list, description="Receipt items")
...
class SpendingAnalytics(BaseModel):
    ...
    transaction_count: int = Field(..., description="Total transaction count")
    average_transaction: float = Field(..., description="Average transaction amount")
    top_categories: List[CategorySpending] = Field(default_factory=list, description="Top spending categories")
    top_merchants: List[MerchantSpending] = Field(default_factory=list, description="Top merchants")
    monthly_trends: Optional[List[MonthlyTrend]] = Field(None, description="Monthly spending trends")


# Response Models
class CustomerResponse(BaseResponse):
    """Customer response model."""
    data: Union[CustomerDetails, List[CustomerDetails]] = Field(..., description="Customer data")


class AccountResponse(BaseResponse):
    """Account response model."""
    data: Union[AccountDetails, AccountSummary, List[AccountDetails]] = Field(..., description="Account data")


class TransactionResponse(BaseResponse):
    """Transaction response model."""
    data: Union[TransactionDetails, List[TransactionDetails]] = Field(..., description="Transaction data")
...
class PaginatedResponse(BaseResponse):
    ...
    data: List[Union[CustomerDetails, AccountDetails, TransactionDetails]] = Field(..., description="Response data")
...
class AccountFilter(BaseModel):
    ...
    min_balance: Optional[float] = Field(None, ge=0, description="Minimum balance filter")
    max_balance: Optional[float] = Field(None, ge=0, description="Maximum balance filter")
