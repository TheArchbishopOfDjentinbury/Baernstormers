...
async def search_products_by_query(query: str, page: int = 1, page_size: int = 10) -> ProductSearchResult:
    """
...
async def find_healthy_alternatives(barcode: str, criteria: str = "nutri_score") -> HealthyAlternativesResult:
    """
    ...
    try:
        ...
        search_result = await search_products_by_query(main_category, page=1, page_size=20)
...
async def analyze_product_nutrition(barcode: str) -> Dict[str, Any]:
    ...
    try:
        ...
        analysis = {
            ...
            "nutrition_facts": product.nutrition_facts.dict() if product.nutrition_facts else None,
...
def _generate_product_recommendations(product: OpenFoodFactsProduct) -> List[str]:
    ...
    if product.nutrition_facts:
        ...
        if nutrition.salt and nutrition.salt > 1.5:
            recommendations.append("Высокое содержание соли. Обратите внимание на потребление натрия")
