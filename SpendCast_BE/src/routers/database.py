    ...
    try:
        ...
        databases = [
            DatabaseStatus(
                ...
                details={k: v for k, v in sql_result.items() if k not in ["status", "database_type", "error"]}
            ),
            DatabaseStatus(
                status=graphdb_result["status"],
                database_type=graphdb_result["database_type"], 
                error=graphdb_result.get("error"),
                details={k: v for k, v in graphdb_result.items() if k not in ["status", "database_type", "error"]}
            )
        ]
