...
async def test_customers_endpoint_integration(client, httpx_mock):
    ...
    httpx_mock.add_response(
        method="POST", url=settings.graphdb_url, json=mock_response, status_code=200
    )
...
async def test_customer_details_integration(client, httpx_mock):
    ...
    accounts_response = {
        "results": {
            "bindings": [
                {
                    "account": {
                        "value": "https://static.rwpz.net/spendcast/alice_checking"
                    },
                    "account_type": {
    ...
    httpx_mock.add_response(
        method="POST", url=settings.graphdb_url, json=accounts_response, status_code=200
    )
...
async def test_customer_transactions_integration(client, httpx_mock):
    ...
    response = client.get(
        "/api/v1/customers/Alice%20Johnson/transactions?limit=10&offset=0"
    )
...
async def test_spending_analysis_integration(client, httpx_mock):
    ...
    httpx_mock.add_response(
        method="POST", url=settings.graphdb_url, json=analysis_response, status_code=200
    )

    response = client.get(
        "/api/v1/customers/Alice%20Johnson/spending-analysis?year=2025"
    )
