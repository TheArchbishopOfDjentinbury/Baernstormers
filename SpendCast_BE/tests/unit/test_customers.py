...
async def test_list_customers_success(client, mock_graphdb_response):
    ...
    with patch('src.routers.customers.execute_sparql_query', new_callable=AsyncMock) as mock_query:
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_get_customer_details_success(client, mock_customer_details_response, mock_customer_accounts_response):
    """Test GET /api/v1/customers/{customer_name} endpoint success."""
    with patch('src.routers.customers.execute_sparql_query', new_callable=AsyncMock) as mock_query:
        # First call returns customer details, second call returns accounts
        mock_query.side_effect = [mock_customer_details_response, mock_customer_accounts_response]
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_get_customer_transactions_with_pagination(client, mock_transactions_response):
    """Test GET /api/v1/customers/{customer_name}/transactions with pagination."""
    with patch('src.routers.customers.execute_sparql_query', new_callable=AsyncMock) as mock_query:
        mock_query.return_value = mock_transactions_response
        
        response = client.get("/api/v1/customers/John%20Doe/transactions?limit=5&offset=10")
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_get_customer_spending_analysis_success(client, mock_spending_analysis_response):
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_get_customer_spending_analysis_with_year(client, mock_spending_analysis_response):
    """Test spending analysis endpoint with custom year."""
    with patch('src.routers.customers.execute_sparql_query', new_callable=AsyncMock) as mock_query:
        mock_query.return_value = mock_spending_analysis_response
        
        response = client.get("/api/v1/customers/John%20Doe/spending-analysis?year=2024")
...
@pytest.mark.unit
@pytest.mark.asyncio
async def test_get_customer_monthly_spending_success(client, mock_monthly_spending_response):
