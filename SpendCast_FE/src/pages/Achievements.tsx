...
const metrics = buildMetrics(); // or however you build metrics
...
// import type { Metrics } from '@/components/achievements/metricfromData';
...
const ACHIEVEMENTS: AchievementCalc[] = [
  ...
  },
  {
    ...
    compute: (m) => {
      const target = 0.3;
      const share = pct(m.healthyFoodSpend, m.healthyFoodSpend + m.unhealthyFoodSpend);
  ...
  },
  {
    ...
    compute: (m) => {
      ...
      const ratio = transportBudget > 0 ? m.transportSpend / transportBudget : 0;
  ...
  },
  {
    ...
    compute: (m) => {
      const share = pct(m.swissMadeSpend, m.swissMadeSpend + m.nonswissMadeSpend);
  ...
  },
  {
    ...
    compute: (m) => {
      ...
      return { progress: Math.round(p), unlocked: m.consecutiveMonthsUnderBudget >= 3 };
    },
  },
];
...
function Achievements({ metrics }: Props) {
  ...
  return (
    <div className="h-full overflow-y-auto p-4 pb-24">
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <h2 className="text-2xl font-semibold text-brand-secondary mb-4">Your Achievements</h2>
          <p className="text-brand-secondary/70">
            Track your progress and unlock rewards as you improve your spending habits
          </p>
        </div>
        ...
        <div className="grid gap-6 grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
          {computed.map((achievement) => {
            const typeStyles = {
              good:   { led: "#22c55e", accent: 'text-green-600',  progress: 'bg-green-500',  border: 'border-green-200',  background: 'bg-green-50' },
              medium: { led: "#f59e0b", accent: 'text-orange-600', progress: 'bg-orange-500', border: 'border-orange-200', background: 'bg-orange-50' },
              bad:    { led: "#ef4444", accent: 'text-red-600',    progress: 'bg-red-500',    border: 'border-red-200',    background: 'bg-red-50' },
            } as const;
    ...
    >                      {animationsLoaded ? (
                  ...
                  <PopoverContent className="w-80 bg-white border border-gray-200 shadow-lg" side="top" align="center">
                    <div className="space-y-3">
                      <div className="flex items-start justify-between">
                        <div>
                          <h4 className="text-lg font-bold text-gray-800">{achievement.title}</h4>
                          <span className={`inline-block text-xs px-2 py-1 rounded-full font-medium ${styles.accent} bg-white border ${styles.border} mt-1`}>
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-xs">
                          ...
                          <span className={`${styles.accent} font-bold`}>{achievement.progress}%</span>
                        </div>
